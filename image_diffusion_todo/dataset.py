...
class AFHQDataModule(object):
    ...
    def _download_dataset(self):
        ...
        # zip_path = os.path.join(self.root, "afhq.zip")
        # os.makedirs(self.root, exist_ok=True)
        # print(f"Downloading AFHQ to {zip_path} …")
        # urllib.request.urlretrieve(URL, zip_path)
        # print("Extracting AFHQ.zip …")
...
if __name__ == "__main__":
    ...
    print("Copying & resizing validation images to data/afhq/eval …")
    for src in data_module.val_ds.fnames:
        dst = eval_dir / Path(src).name
        shutil.copy2(src, dst)
        img = Image.open(dst).resize((64, 64))
        img.save(dst)
        print("  copied", dst.name)

    print(f"Finished building eval directory at {eval_dir}")    
